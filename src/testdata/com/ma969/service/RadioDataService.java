...
public class RadioDataService {
	...
	public List<RadioData> listRadioDataByParam(Map map) throws Exception {
		...
		if (!Help.isEmpty(radioDatas)) {
			radioDatas.forEach(radioData -> {
				Transmit transmit = transmitMapper.findById(radioData.getEid());
	...
	@SuppressWarnings("rawtypes")
	public boolean readerFileToRadioData(String fileName, String area, int eid) {
		int addRadioData = 0;
		try {
			String encoding = "GBK";
				...
				readfile: while (lineTxt != null) {
					if (t >= 1) {
						...
						if (radioDataMapper.checkTime(pointarr[0]) == 0) {
							...
							radioData.setLongitude(Float.parseFloat(pointarr[1]));
							radioData.setLatitude(Float.parseFloat(pointarr[2]));
							...
							radioData.setArea(area);
							radioData.setEid(eid);
							if (radioData != null) {
	...
	public List<DataMode> listDataModes() {

		return dataModeMapper.listDataModes();
	}
